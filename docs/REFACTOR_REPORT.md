...
The repository-wide implementation and every bridge-free/local-Tk validation phase are complete. A bounded real-game gate passed at the `c7d38bc` live boundary with DLL `f6be2f86...`; the later parity-tested simplifications and current DLL `3b849aa4...` were not reinstalled or re-exercised in the game. Separate compatibility, concurrency, refactoring-quality, performance, and final-diff reviews were run, and their high-confidence findings were resolved where the environment allowed.

The overall goal is not labeled complete. The live-gate bridge was exercised in the real game for startup, menu/seed/gameplay entry, observation and action semantics, repeated loss/retry reset, fusion, and a corrected protected Generalist evaluation. Short fresh/resumed fixed training, fixed Level-3 evaluation, win/timeout/reward/unlock/replay/advancement traces, current-source live confirmation, live game-backed GUI operation, and measured live latency remain outstanding. The cross-run approximately-5% performance gate also remains unresolved, and first-party runtime code increased from 48,127 to 54,821 physical lines rather than achieving a net reduction.
...
- All six protected model/metadata sizes, mtimes, and SHA-256 values exactly match the Phase 5 table. The installed recovery DLL was preserved before the live gate, the deterministic live-gate DLL `f6be2f86...` was installed temporarily for validation, and the original DLL was restored and reverified at `5643ec37984762ab72fea3c50e87fcc466b905c5cf046c307fdaa6e0ce42a0f4`.
...
- The installed recovery DLL was copied to `runs/manual_phase8/final_live_gate/PvZRLBridge.recovery.dll` and verified at SHA-256 `5643ec37984762ab72fea3c50e87fcc466b905c5cf046c307fdaa6e0ce42a0f4` before the deterministic `f6be2f86...` live-gate DLL was installed. That bridge loaded and accepted connections on port 32323; after the gate the recovery DLL was restored to `Game Files/Mods`, its hash was reverified, and the listener was closed. The current `3b849aa4...` DLL has only automated/parity evidence.
...
- The fixed/identity SB3 mask projection now copies index-preserving raw masks directly while retaining the shifted `dynamic_14` loop. A session-local alternating exact-output A/B measured fixed projection `0.01960 -> 0.00850 ms` median (2.31x) and identity projection `0.39195 -> 0.03970 ms` (9.87x); the ad hoc microbenchmark was independently reviewed but not retained as a repository artifact. In the full retained current benchmark, fixed/identity mask medians are `0.84485/1.30230 ms`; compared with the immediately preceding same-machine lifecycle-source run they are 6.0% and 22.4% lower, but that cross-process comparison is supporting evidence rather than a causal claim.
- Evaluation aggregation was benchmarked after consolidation because the first canonical-row version doubled a 100-log synthetic workload. A session-local HEAD-versus-working-tree run measured old `2.3314 s`, new `2.3038 s` across 200 complete summaries (1.2% faster); it was independently reviewed but not retained as a repository artifact. The retained implementation passed 1,000 randomized whole-summary differentials for contract-valid `EvalLog` dictionaries, including malformed values within valid string-key dictionaries. Arbitrary out-of-contract container/key/scalar corruption is not claimed equivalent.
...
The raw removal count is not a code-reduction claim: 10,742 C# method-body lines were moved during the bridge split, and most of Phase 7's 1,862-line GUI deletion was relocated into focused modules. The final physical total is authoritative. Removed-name counts include only definitions deleted without a replacement under the same symbol; move-only class extraction and retained import/alias surfaces are excluded. Duplicate-family counts add one when multiple subsystem-specific bodies or schema lists become one named authority, regardless of the number of deleted copies. Under that method, the 22 families cover request ownership, JSONL tailing, registry/configuration, action decisions, fusion validation/execution, per-observation facts, rewards and reward schema, lane/safety diagnostics, episode telemetry and evaluation reducers, live-status writing, lifecycle predicates, Generalist progression, bridge occupancy/lanes/cards/type counts, GUI status/commands/process/logs, coach queue/aliases, and live fusion-test setup. The 12 narrowly counted compatibility surfaces are four registry projections, reward/reset adapters, two deprecated fusion views, three SB3 state projections, and the compatibility fusion-candidate factory.
...
The `c7d38bc`/`f6be2f86...` live-boundary install/startup and bounded environment checks above ran, and the original installed recovery DLL was restored and reverified. The current `3b849aa4...` DLL remains automated-only. The commands below are retained as reproducibility evidence or remain required where explicitly marked.
